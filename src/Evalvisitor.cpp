		...
		if (ctx -> trailer() -> arglist() != nullptr)
			for (int i = 0 , tot = (int)ctx -> trailer() -> arglist() -> argument().size();i < tot;++ i)
				if (ctx -> trailer() -> arglist() -> argument()[i] -> ASSIGN() == nullptr)
				{
					...
					name_space::is_global_block = 0 , name_space::crt_flag = 0 , new_name_space[func_arglist[i].first] = tmp , name_space::crt_flag = 1 , name_space::is_global_block = was_global_block , ext[func_arglist[i].first] = 1;
				}
				else
				{
